...
//
function addHighScore(record, node, currentName) {
    ...
    // Set the attributes and create the text
    score.setAttribute("x", 390);
    score.appendChild(svgdoc.createTextNode(record.score));

    if(record.name == currentName) {
        name.setAttribute("style","fill:crimson");
        score.setAttribute("style","fill:crimson");
    }
...
//
function showHighScoreTable(table, currentName) {
    // Show the table
    ...
    for (var i = 0; i < 5; i++) {
        ...
        addHighScore(table[i], node, currentName);
    }
}
